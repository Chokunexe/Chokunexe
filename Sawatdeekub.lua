    ...
    while wait() do
        pcall(function()
            if _G.AutoFarm then
                ...
                if questData then
                    if not UIQ.Visible or UIQ.Visible == false then
                        -- Tween ไปยังตำแหน่ง CFrameQ ด้วยความเร็ว 300
                        ...
                        if enemy then
                            repeat
                                if enemy 
                                    ...
                                    -- Tween ไปยังมอนสเตอร์ด้วยความเร็ว 300
                                    TW(enemy:FindFirstChild("HumanoidRootPart").CFrame * CFrame.new(0, 30, 0))
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                else
                                    -- เคลื่อนที่ไปยังตำแหน่งของมอนสเตอร์
                                    Char.HumanoidRootPart.CFrame = questData.CFrameMon * CFrame.new(0, 30, 0)
                                end
                                wait(1)
                                enemy = game:GetService("Workspace").Enemies:FindFirstChild(questData.Mon)
...
-- ฟังก์ชัน Tween ที่ปรับความเร็วเป็น 300 หน่วยต่อวินาที
function TW(targetCFrame)
    local tweenService = game:GetService("TweenService")
    local char = Char.HumanoidRootPart
    local distance = (char.Position - targetCFrame.Position).Magnitude
    local speed = 300 -- ความเร็ว 300 หน่วยต่อวินาที
    local time = distance / speed -- เวลา = ระยะทาง / ความเร็ว

    -- สร้าง Tween
    local tweenInfo = TweenInfo.new(time, Enum.EasingStyle.Linear)
    local goal = {CFrame = targetCFrame}
    local tween = tweenService:Create(char, tweenInfo, goal)
    tween:Play()
